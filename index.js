...
function filterUnfundedOnly(games) {
    ...
    let listUnderfundedGames = games.filter((game) => {
        return game.pledged < game.goal
    })

    // use the function we previously created to add the unfunded games to the DOM
    addGamesToPage(listUnderfundedGames)
...
function filterFundedOnly(games) {
    ...
    // use filter() to get a list of games that have not yet met their goal
    let listFundedGames = games.filter((game) => {
        return game.pledged >= game.goal
    })
...
let descString = `A total of $${totalPledged.toLocaleString()} has been raised for ${GAMES_JSON.length} games. ${numUnfundedGames === 1 ? "Currently, 1 game remains unfunded. We need your help to fund this amazing game!" : `Currently, ${numUnfundedGames} games remain unfunded. We need your help to fund these amazing games!`}`;
